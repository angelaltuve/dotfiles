...
say() {
  espeak-ng "$1" --stdout | aplay &>/dev/null
}
...
timer() {
  ...
  for ((min = MIN; min >= 0; min--)); do
    for ((sec = SEC; sec > 0; sec--)); do
      BODY="Time Remaining: $(formatTime $min 1):$(formatTime $sec) seconds"
      OUTPUT=$(
        timeout 1 notify-send "$2" "$BODY" -r "$3" -u low -a "$NAME" -i "$ICON" -A "Pause" -A "Stop" -A "Skip" &
        sleep 1
      )
      case $OUTPUT in
      0)
        ...
        while true; do
          OUT=$(
            timeout 1 notify-send "Paused!" "$BODY" -r "$3" -u critical -a "$NAME" -i "$ICON" -A "Resume" &
            sleep 1
          )
          if [[ $OUT = 0 ]]; then
            break
